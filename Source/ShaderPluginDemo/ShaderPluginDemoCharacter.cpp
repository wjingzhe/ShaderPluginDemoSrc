...
void AShaderPluginDemoCharacter::BeginPlay()
{
	...
	PtrRSBlendQuadShaderModule = new FRSBlendQuadShaderModule(1024,1024,GetWorld()->Scene->GetFeatureLevel());
	FRHIResourceCreateInfo CreateInfo;
	NewInputTexture = RHICreateTexture2D(1024, 1024, PF_R32G32B32A32_UINT, 1, 1, TexCreate_ShaderResource | TexCreate_UAV, CreateInfo);

	NewInputTextureSRV = RHICreateShaderResourceView(NewInputTexture, 0);
}
...
void AShaderPluginDemoCharacter::BeginDestroy()
{
	...
	if (NewInputTexture)
	{
		NewInputTexture.SafeRelease();
		NewInputTexture = nullptr;
	}
	if (NewInputTextureSRV)
	{
		NewInputTextureSRV.SafeRelease();
		NewInputTextureSRV = nullptr;
	}
}
...
void AShaderPluginDemoCharacter::Tick(float DeltaSeconds)
{
	...
	if (PtrRSBlendQuadShaderModule)
	{
		PtrRSBlendQuadShaderModule->ExecutePixelShader(NewInputTexture, NewInputTextureSRV);
	}
